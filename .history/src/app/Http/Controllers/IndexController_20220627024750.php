...
class IndexController extends Controller
{
    public function index()
    {
        ...
        $lang = StudyData::leftJoin('study_languages', 'study_data.language_id', '=', 'study_languages.id')
            ->select('study_languages.language', DB::raw("SUM(study_data.study_hour) as sum"), 'study_languages.color')
            ->groupBy('study_languages.language', 'study_languages.color')
            ->get();

        // 円グラフ（内容）
        $content = StudyData::leftJoin('study_contents', 'study_data.content_id', '=', 'study_contents.id')
            ->select('study_contents.content', DB::raw("SUM(study_data.study_time) as sum"))
